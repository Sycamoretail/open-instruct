...
→ RLVR parquet             → SFT trajectories             → 策略更新
...
```bash
# 默认：Qwen3-8B + sft_distill_score.jsonl，8 GPU + Deepspeed Stage3
./scripts/train/scholar/sft_qwen3_8b.sh

# 用别的 base / 别的数据 / 别的输出目录
BASE_MODEL_NAME_OR_PATH=Qwen/Qwen3-0.6B \
SFT_DATA_PATH=data/scholar/sft_distill_pilot.jsonl \
NUM_PROCESSES=1 \
...
WANDB_MODE`。`MAX_SEQ_LENGTH` 默认 16384，与 RL 端 `--pack_length 16384`
对齐。
...
**6.2.1 [scholar_tools.py::_ScholarToolBase.reset](file:///mlx_devbox/users/luoyunze/playground/open-instruct/open_instruct/environments/tools/scholar_tools.py#L77-L112)**
...
**6.2.2 [prepare_scholar_data.py::_build_env_config](file:///mlx_devbox/users/luoyunze/playground/open-instruct/scripts/data/scholar/prepare_scholar_data.py#L142-L231)**
...
- [scripts/data/scholar/distill_sft_trajectories.py](file:///mlx_devbox/users/luoyunze/playground/open-instruct/scripts/data/scholar/distill_sft_trajectories.py)
  GPT-5.x teacher 蒸馏 SFT trajectories，支持 per-type reward 过滤
...
| 症状 | 原因 | 处理 |
|------|------|------|
| scholar search 召回目标 paper 本身 | `curr_title` 没生效 | 检查 parquet 是否有 `env_config` 列（§6.2.2）；检查 `_ScholarToolBase.reset()` 是否被覆盖（§6.2.1） |
| `last_time` 全部等于 distill 的 `--last-time` 默认值 | distill 走了老版本硬编码 | 确保脚本包含 `_extract_sample_tool_context`（§6.1） |
| Azure 报 `max_completion_tokens` 不够 | GPT-5 reasoning tokens 也计数 | 调大 `--max-completion-tokens`（默认已 8000） |
| RACE "Failed to extract JSON" | judge 偶尔输出 Markdown prose | 已内置 Method 7 兜底（§5.2），若仍失败先用 `debug_race_single.py` 复现 |
| RL rollout 每条都复用同一个 `page_idx` | `_reset_internal_state` 未被调用 | 确认 RL 侧修复已生效（§6.2.1） |
...
- **DRB run_eval.py**：`SCORE_PROMPT_EN` / `SCORE_PROMPT_ZH` 各加 7 条
  CRITICAL OUTPUT RULES（方案 B）；保留 Method 7 Markdown 兜底（方案 A）。
- **ping_judge_and_teacher.py**：新增（或恢复）探活脚本。
- **distill_sft_trajectories.py**：scholar_evaluator 集成、per-type 阈值、
  `--always-score`、async scoring、`_extract_sample_tool_context`、续跑优化。
- **prepare_scholar_data.py**：新增 `_build_env_config` + 4 个 env 的 kwargs 写入。
- **scholar_tools.py**：`_ScholarToolBase` 覆盖 `reset(**kwargs)`，显式调用
  `tools._reset_internal_state`。
- **data/scholar_en_rlvr/**：全部 5 份 parquet 已按新逻辑重新生成，带
  `env_config` 列。
