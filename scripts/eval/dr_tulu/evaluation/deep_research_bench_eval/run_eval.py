...
Environment Variables:
    OpenAI-compatible backend (default):
        DRB_JUDGE_API_BASE: Base URL ending with /v1 (or without /v1)
        DRB_JUDGE_API_KEY: Optional API key for the judge endpoint
        DRB_RACE_MODEL: Model name for RACE judging
        DRB_FACT_MODEL: Model name for FACT validation
        DRB_CLEAN_MODEL: Optional model name for article cleaning (defaults to DRB_RACE_MODEL)

    Internal HTTP backend (same style as scholar_evaluator.py):
        DRB_JUDGE_BACKEND=internal_http
        DRB_INTERNAL_JUDGE_URL: Judge endpoint URL
        DRB_INTERNAL_PROXY: Optional HTTP proxy

    Optional Gemini backend:
        DRB_JUDGE_BACKEND=gemini
...
def extract_json_from_markdown(text: str) -> Optional[str]:
    ...
    # Pre-clean: 很多 judge 模型会把 JSON 包在 <answer>/<output>/<json> 标签里，
    # 或在前面加一大段 <think>...</think> reasoning。把这些壳剥掉，让后面的
    # 各种 extract 策略能命中。
    original = text
    # 剥掉 <think>...</think>（包括中文变体）。
    for think_tag in ("think", "thinking", "思考", "reasoning"):
        text = re.sub(
            rf"<\s*{think_tag}\s*>.*?<\s*/\s*{think_tag}\s*>",
            "",
            text,
            flags=re.DOTALL | re.IGNORECASE,
        )
    # 优先取 <answer>/<output>/<json>/<result> 里的内容。
    for wrap_tag in ("answer", "output", "json", "result", "回答"):
        m = re.search(
            rf"<\s*{wrap_tag}\s*>(.*?)<\s*/\s*{wrap_tag}\s*>",
            text,
            flags=re.DOTALL | re.IGNORECASE,
        )
        if m:
    ...
    # Method 7: Markdown-prose fallback.
    # 有些 judge 模型完全不输出 JSON，而是用 Markdown 中文/英文叙述 + "得 X 分"/"Score: X"。
    # 例如：
    #   ### 全面性
    #    - **文章1**：... 得9分。
    #    - **文章2**：... 得6分。
    # 我们直接从这种结构里抽四个维度的分数，合成最小 JSON。
    md_result = _parse_markdown_scores(text)
    if md_result is not None:
        return json.dumps(md_result)

    return None


_DIM_ALIASES: dict[str, list[str]] = {
    "comprehensiveness": ["comprehensiveness", "全面性"],
    "insight": ["insight", "洞察力", "洞察"],
    "instruction_following": [
        "instruction_following",
...
def _parse_markdown_scores(text: str) -> Optional[dict]:
    """从 Markdown 散文式 judge 回复里提取每个维度的 article_1/article_2 分数。
    ...
    成功时返回::
        {"comprehensiveness": [{"criterion": "...", "article_1_score": 9, "article_2_score": 6}],
         "insight": [...], ...}
    四个维度至少命中一个才算成功。
    """
    # 尽量从 heading 到下个 heading 切块。
    # 先把文本按行粗切；段之间用空行或 "### "/"## "/"# "/"**xx**" 开头当 delimiter。
    ...
    for canonical, aliases in _DIM_ALIASES.items():
        ...
        for alias in aliases:
            ...
            if pos != -1 and (dim_pos == -1 or pos < dim_pos):
                dim_pos = pos
                dim_len = len(alias)
        if dim_pos == -1:
            continue
        ...
        # 切到下一个维度起点之前
        next_pos = len(text)
        for other_canonical, other_aliases in _DIM_ALIASES.items():
            if other_canonical == canonical:
                continue
            for other_alias in other_aliases:
                p = lower.find(other_alias.lower(), dim_pos + dim_len)
                if p != -1 and p < next_pos:
                    next_pos = p
        section = text[dim_pos:next_pos]

        a1 = _extract_article_score(section, article_idx=1)
        a2 = _extract_article_score(section, article_idx=2)
        if a1 is not None or a2 is not None:
            dim_scores[canonical] = {
                "article_1_score": a1 if a1 is not None else 0.0,
                "article_2_score": a2 if a2 is not None else 0.0,
            }

    if not dim_scores:
        return None

    # 每个命中的维度合成一条 criterion。没命中的维度补全 0 占位，
    # 让 calculate_weighted_scores 不因为 key 缺失而爆。
    result: dict = {}
    for canonical in _DIM_ALIASES:
        if canonical in dim_scores:
            s = dim_scores[canonical]
            result[canonical] = [
                {
                    "criterion": f"{canonical} (parsed from prose)",
                    "article_1_score": float(s["article_1_score"]),
                    "article_2_score": float(s["article_2_score"]),
                }
            ]
        else:
            result[canonical] = [
                {
                    "criterion": f"{canonical} (missing)",
                    "article_1_score": 0.0,
                    "article_2_score": 0.0,
                }
            ]
...
def _extract_article_score(section: str, article_idx: int) -> Optional[float]:
    """从 section 文本里提取 ``文章{idx}`` 或 ``Article {idx}`` 那一行上的分数。

    支持：``得9分`` / ``得 9 分`` / ``9/10`` / ``Score: 8.5`` / ``scored 9`` 等。
    """
    # 匹配包含 "文章1" / "Article 1" 的一整行（或到下一行 "文章2" 之前的块）。
    patterns_anchor = [
        rf"文章\s*{article_idx}[^\n]*",
        rf"article[\s_\-]*{article_idx}[^\n]*",
    ]
    candidate = None
    for pat in patterns_anchor:
        ...
        if m:
            ...
            # 顺便吃到下一个锚点之前，提高分数捕获率。
            start = m.end()
            next_anchor = len(section)
            for next_idx in (1, 2, 3):
                if next_idx == article_idx:
                    continue
                for next_pat in (
                    rf"文章\s*{next_idx}",
                    rf"article[\s_\-]*{next_idx}",
                ):
                    m2 = re.search(next_pat, section[start:], flags=re.IGNORECASE)
                    if m2:
                        p = start + m2.start()
                        if p < next_anchor:
                            next_anchor = p
            candidate = section[m.start() : next_anchor]
            break
    ...
    for p in score_patterns:
        m = re.search(p, candidate, flags=re.IGNORECASE)
        if m:
            try:
                val = float(m.group(1))
                if 0 <= val <= 10:
...
class JudgeLLMClient:
    """Judge client for DRB evaluation.

    Supports:
    - `openai_compatible` chat completions API (default)
    - `internal_http` endpoint compatible with scholar_evaluator.py
    ...
    def __init__(
        ...
        *,
        ...
        self.fact_model = fact_model or os.environ.get("DRB_FACT_MODEL")
        self.clean_model = clean_model or os.environ.get("DRB_CLEAN_MODEL")
        ...
        if self.backend == "gemini":
            try:
                ...
                raise ImportError(
                    "google-genai package required for Gemini backend. Install with: pip install google-genai"
                )
            ...
            self.fact_model = self.fact_model or DEFAULT_GEMINI_FACT_MODEL
            self.clean_model = self.clean_model or self.race_model
            return
        ...
        if self.backend == "internal_http":
            ...
            # internal_http endpoint 已经在 URL 里绑定了后端模型，
            # 这里的 model 名只作为 payload 的 label/日志标识，给个默认即可。
            self.race_model = self.race_model or "internal_http"
            self.fact_model = self.fact_model or self.race_model
            self.clean_model = self.clean_model or self.race_model
            return

        if self.backend != "openai_compatible":
            raise ValueError(
                f"Unsupported backend: {self.backend}. Use 'openai_compatible', 'internal_http', or 'gemini'."
            )
        ...
        if not self.api_base:
            raise ValueError(
                "OpenAI-compatible judge backend requires DRB_JUDGE_API_BASE (or OPENAI_BASE_URL / OPENAI_API_BASE)."
            )
...
**CRITICAL OUTPUT RULES (must follow, no exceptions):**
1. Your entire response MUST be a single JSON object that matches `<output_format>` exactly.
2. Do NOT write any narrative, analysis, markdown headings, bullet points, or summary text outside of the JSON.
3. Do NOT wrap the JSON in any tags such as `<answer>`, `<output>`, `<think>`, or `<回答>`.
4. Wrap the JSON in a single fenced code block starting with ```json and ending with ```.
5. All four top-level keys (`comprehensiveness`, `insight`, `instruction_following`, `readability`) MUST be present, each as a non-empty list of criterion objects.
6. Every criterion object MUST contain `criterion`, `analysis`, `article_1_score`, `article_2_score`; scores MUST be numeric (not strings, not "9分").
7. The JSON MUST be directly parseable by `json.loads`. No trailing commas, no comments, no Chinese full-width punctuation inside JSON values that would break parsing.

Now output ONLY the JSON (inside one ```json fenced block) evaluating the two articles.
</user_prompt>
"""
...
**关键输出规则（必须严格遵守，无任何例外）：**
1. 你的**整个回复**必须是一个 JSON 对象，结构与 `<output_format>` 完全一致。
2. **不要**输出任何 JSON 之外的叙述、分析、Markdown 标题（如 `### 全面性`）、列表、总结或综合评分段落。
3. **不要**把 JSON 包在任何标签里（如 `<answer>`、`<output>`、`<think>`、`<回答>`）。
4. 必须把 JSON 放在一个 ```json 代码块内，以 ```json 开头、以 ``` 结尾。
5. 四个顶层 key（`comprehensiveness`、`insight`、`instruction_following`、`readability`）必须全部存在，每个都是非空的 criterion 对象列表。
6. 每个 criterion 对象必须包含 `criterion`、`analysis`、`article_1_score`、`article_2_score`；分数必须是**数字**（例如 `9` 或 `8.5`），**不能**是字符串或 `"9分"` 这种形式。
7. JSON 必须能被 `json.loads` 直接解析：无尾随逗号、无注释、JSON value 内不要出现会破坏解析的全角标点。

请现在**只**输出一个 ```json 代码块，内部是对两篇文章的评估 JSON，不要输出其他任何内容。
</user_prompt>
"""
...
class ArticleCleaner:
    ...
    def __init__(self, clean_agent: JudgeLLMClient):
        ...
        for retry in range(max_retries):
            try:
                result = self.clean_agent.generate(
                    ...
                    system_prompt="",
                    model=self.clean_agent.clean_model,
                )
...
def process_single_race_item(
    ...
    while retry_count < max_retries and not success:
        try:
            ...
            last_response_snippet = (llm_response_str or "")[:1200]
            ...
            if retry_count < max_retries:
                ...
                logger.error(
                    f"ID {task_id}: Failed after {max_retries} retries - {last_error_repr}"
                )
...
# FACT Evaluation
# ============================================================================
def validate_single_citation(data: tuple, id_to_lang_map: dict, llm_client: JudgeLLMClient) -> dict:
    ...
    lookup_id = str(article_id) if article_id is not None else None
    if not lookup_id or lookup_id not in id_to_lang_map:
        return {"url": url, "validate_res": [], "error": "Language not found"}

    lang = id_to_lang_map[lookup_id]
...
def run_fact_evaluation(
    ...
    # 注意：scraped.jsonl 里的 ``id`` 是 str（来自 example_id），而 query.jsonl 里的
    # ``id`` 是 int。这里统一转成 str 作为 key，避免类型 mismatch 导致
    # ``Language not found``。
    query_data = load_jsonl(str(QUERY_FILE))
    id_to_lang_map = {
        str(item["id"]): item.get("language")
    ...
    if not os.path.exists(validated_path):
        logger.warning(
            "FACT validation produced no validated output at %s. "
            "This usually means the input file had no usable examples, or every example was missing an `id`.",
            validated_path,
        )
        return None
...
""",
    ...
    parser.add_argument(
        ...
        help="Judge backend for cleaning/RACE/FACT (default: openai_compatible)",
    )
    parser.add_argument(
        "--llm_api_base",
        type=str,
    ...
    parser.add_argument(
        "--race_model",
        type=str,
        default=os.environ.get("DRB_RACE_MODEL")
        or os.environ.get("DRB_JUDGE_MODEL")
        or (DEFAULT_GEMINI_RACE_MODEL if os.environ.get("DRB_JUDGE_BACKEND") == "gemini" else None),
        help="Model used for RACE scoring",
    )
    parser.add_argument(
        "--fact_model",
        type=str,
        default=os.environ.get("DRB_FACT_MODEL"),
        help="Model used for FACT validation (defaults to race_model)",
    )
    parser.add_argument(
        "--clean_model",
        type=str,
        default=os.environ.get("DRB_CLEAN_MODEL"),
        help="Model used for article cleaning (defaults to race_model)",
    )
