...
def clean_rich_media_reference(text: str, return_citation_ids: bool = False):
    """
    将 <RichMediaReference>...</RichMediaReference> 中的 superscript 引用
    统一替换为中括号格式：[1][2][3]

    支持格式：
    1) <|superscript|>:1
    2) superscript:1
    3) 上述两种格式的任意组合与重复，支持逗号分隔
    ...
    def _replace(match):
        ...
        return re.sub(
            r"(?:<\|superscript\|>|superscript):(\d+)",
            r"[\1]",
            inner.replace(",", " ")
        )

    cleaned_text = re.sub(
        r"<RichMediaReference>(.*?)</RichMediaReference>",
        _replace,
        text,
        flags=re.DOTALL
    )
...
FAAS_CONFIG_LST = ['https://78wyfcwp.fn.bytedance.net/arxiv_fetch',
...
async def relevance_rate_evaluate(paper_title: str, section_title: str, gen_references: list[dict], text: str) -> tuple[float, bool]:
    """
    评估生成的相关工作中引用的论文是否相关。
    ...
    for ref in gen_references:
        ...
        try:
            prompt = prompt_template.format(paper_title=paper_title, section_title=section_title, text=text, ref_title=ref_title, ref_abstract=ref_abstract)
            content = sp + prompt
...
async def citation_precision_evaluate(gen_response: str, gen_references: list[dict]) -> tuple[float, bool]:
    ...
    try:
        ...
        sentences = custom_sent_tokenize_multilingual(gen_response)
    except Exception as e:
        print(f"CitationPrecisionEvaluator Error while custom_sent_tokenize: {e}")
        return 0.0, False

    prompt_template = """### Reference Title\n{ref_title}\n\n### Reference Abstract\n{ref_abstract}"""
    references_prompt_dict = {int(ref["id"]): prompt_template.format(ref_title=ref["title"], ref_abstract=ref["abstract"]) for ref in gen_references}
        ...
        for r in ref:
            if r in references_prompt_dict:
                ...
                try:
                    ...
                    print(f"CitationPrecisionEvaluator Error processing sentence: {sentence}, ref: {r}, response: {response}, error: {e}")
                    is_valid = False
                    continue
            else:
    ...
    if not citation_precision_scores:
        print(f"CitationPrecisionEvaluator Error: No citation precision scores computed for gen_response:\n{gen_response}")
        return 0.0, False
...
async def reference_appearance_evaluate(gen_response: str, gen_references: list[dict]) -> tuple[float, bool]:
    ...
    if not appearance_score_list:
        print(f"ReferenceAppearanceEvaluator Error: No reference appearance scores computed for gen_response:\n{gen_response}")
        return 0.0, False
...
async def evaluate_scholar_score_verifier(candidate: str, ground_truth: dict, tool_results_str: str) -> dict:
    ...
    if ground_truth['data_type'] == 'understanding':
        ...
        score_weight = {
            'nugget_coverage_score': 5.,
            'reference_coverage_score': 3.,
        }
        ...
        if candidate is None:
            ...
            result_dict['reference_coverage_score'], _ = await reference_coverage_evaluate(gen_references, citations_list)
            ...
            if os.getenv('PT_DEBUG', None):
                print(f'[PT] [scholar_score_verifier] judge {repr(gen_response[:100])} against ground truth {repr(answer[:100])}: {score_str}')
        ...
        print(f'query: {ground_truth["verifiable_meta"]["query"]}')
        paper_title = ground_truth['verifiable_meta']['paper_title']
        ...
        score_weight = {
            'relevance_rate_score': 2.,
            'nugget_coverage_score': 7.,
            'reference_coverage_score': 10.,
            'citation_precision_score': 1.,
        }
        ...
        if candidate is None:
            ...
            score = int(SCORE_PENALTY)
            result_dict['relevance_rate_score'], _ = await relevance_rate_evaluate(paper_title, section_title, gen_references, text)
            result_dict['reference_coverage_score'], _ = await reference_coverage_evaluate(gen_references, citations_list)
            result_dict['nugget_coverage_score'], _ = await nugget_coverage_evaluate(nuggets, gen_response)
            result_dict['citation_precision_score'], _ = await citation_precision_evaluate(gen_response, gen_references)
            ...
            if os.getenv('PT_DEBUG', None):
                print(f'[PT] [scholar_score_verifier] judge {repr(gen_response[:100])} against ground truth {repr(text[:100])}: {score_str}')
...
async def _eval_nuggets_objective(query: str, nuggets: list[dict], candidate_with_references: str) -> dict[str, int | None]:
...
async def _eval_nuggets_subjective(query: str, nuggets: list[dict], candidate_with_references: str) -> dict[str, int | None]:
    ...
    for seq_idx, nugget in enumerate(subj_nuggets):
        ...
        nuggets_lines.append(
            json.dumps({"要点编号": rubrics_idx, "要点类型": rubrics_type, "要点内容": nugget['要点']}, ensure_ascii=False)
        )
    nuggets_text = "\n".join(nuggets_lines)
    ...
    try:
        ...
        for item in parsed_items:
            ...
            nugget_cls = subj_nuggets[rubrics_idx - 1].get('重要性分类', '必要点') if rubrics_idx <= len(subj_nuggets) else '必要点'
...
async def evaluate_scholar_score_eval_verifier(candidate: str, ground_truth: dict, tool_results_str: str) -> dict:
    ...
    def _compute_round_score(round_results: dict[str, int | None], all_rounds: list[dict[str, int | None]], target_nuggets: list[dict]) -> float | None:
    ...
    if subj_nuggets:
        ...
        subj_final = subj_scores[0] if len(subj_scores) == 1 else (_majority_vote([round(s, 6) for s in subj_scores]) if subj_scores else 0.0)
    else:
        subj_scores = []
        subj_final = 0.0
